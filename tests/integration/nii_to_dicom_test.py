...
def test_dataset_nii_crawler_direct(nii_test_dataset_path: str) -> None:
    ...
    organ_extractor = ps_fio.SimpleOrganExtractor(('Cochlea', 'Skull', 'TV', 'Vol2016',
                                                   'Cochlea', 'AN', 'T1+ 13', 'Vol2015', 'tv', 'Vol2018'))
    annotator_extractor = ps_fio.SimpleAnnotatorExtractor(('NA',))

    # construct the crawler
    crawler = ps_fio.DatasetFileCrawler(nii_test_dataset_path,
                                        '.nii.gz',
                                        modality_extractor,
                                        organ_extractor,
                                        annotator_extractor)
    ...
    for series_info_entry in series_info:
        image_series_info = [entry for entry in series_info_entry
                             if isinstance(entry, ps_fio.IntensityFileSeriesInfo)]
        assert len(image_series_info) == 2

        seg_series_info = [entry for entry in series_info_entry
                           if isinstance(entry, ps_fio.SegmentationFileSeriesInfo)]
...
def test_dataset_nii_crawler_iterative(nii_test_dataset_path: str) -> None:
    ...
    for series_info in crawler:
        ...
        seg_series_info = [entry for entry in series_info
                           if isinstance(entry, ps_fio.SegmentationFileSeriesInfo)]
        assert len(seg_series_info) >= 3
